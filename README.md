...
```sh
docker build -t docker_addressservice -f Dockerfile .
```
...
```sh
docker run --rm -t -p 8080:8080 --mount type=bind,source="$PWD"/config.yml,target=/app/config.yml,readonly docker_addressservice:latest /bin/AddressService /app/config.yml
```
